...
def plot_tf(ax, times_called, filepath, args):
    ...
    if args.print_final_stresses:
        ...
        if single_coils:
            ...
            for i in range(4):
                if neighbour_shell_averages:
                    ...
                    xdata = np.mean(xdict['raw_data'][:,:],axis=1)
                    #print (xdata, xdict['raw_data'][:,:])
        ...
        try:
            ...
            if args.fit2: fit2 = fit_data(ax, xdata, ydata, args.fit2_range, data_label+' fit 2', args)
            ...
            if args.fit or args.fit2:
                ...
                if args.fit2:
                    if args.key_shell or args.key_pole:
                        fit_text += 'Fitted2 initial thickness = {:2.2f}'.format(fit2.r[0])+' mm\n'
                        fit_text += 'Fitted2 slope = {:2.0f}'.format(fit2[1]) + ' MPa/mm\n'
                    else: 
                        fit_text += 'Fitted2 initial stress = {:2.2f}'.format(fit2.r[0])+' MPa\n'
                        fit_text += 'Fitted2 slope = {:2.2f}'.format(fit2[1]) + ' MPa/MPa\n'
